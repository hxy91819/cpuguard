...
关于“进程暂未启动”的行为：
- `watch` 由 `launchd` 托管 `cpuguard` 内置 runner，而不是直接托管等待中的 `cpulimit -e`。
- runner 会低频等待目标进程出现，命中后再启动 `cpulimit -p <pid>`；目标退出后由 `launchd` 重新拉起下一轮等待。
- 因此重启场景下规则仍可恢复，同时避免空跑的 `cpulimit -e` 常驻占用 CPU。
...
- 交互时输入序号执行限制，输入 `k<序号>` 终止单个可疑孤儿进程，输入 `x<序号>` 批量终止当前快照中同名异常进程，输入 `q` 退出，直接回车立即刷新。
