...
6. 在 top 视图中自动检测可疑孤儿进程（PPID=1 + CPU≥阈值 + 运行≥30min + 非系统进程），并支持交互式终止。
7. 在 top 视图中支持按当前快照批量终止同名异常进程（需确认，且禁止对系统进程触发）。
...
7. `top` 交互式批量终止仅作用于当前表格快照中与所选条目 `NAME` 完全相同的进程，不做全局模糊匹配。
...
- `watch` 与重启恢复依赖 `launchd` 托管的 `cpuguard` runner；runner 低频等待目标，命中后启动 `cpulimit -p`，不直接托管等待中的 `cpulimit -e`。
- `top` 的批量终止只允许命中当前快照中的非系统进程，且必须经过显式确认。
